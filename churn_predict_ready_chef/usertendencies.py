...
for name,group in groups:
    print name
    group['visit_date'] = group['visit_date'].apply(pd.to_datetime)
    index = DatetimeIndex(group['visit_date'])
    group['visit_date'].index = index
    print rolling_mean(group['visit_date'],window='24h')
